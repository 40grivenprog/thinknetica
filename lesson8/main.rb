...
class Visual
  ...
  def route_for_train
    return puts 'Что бы задать маршрут поезду необходимо иметь один поезд и один маршрут' if @routes.empty? || @trains.empty?
  ...
  def stations_method
    return puts 'У вас нет маршрутов или недостаточно станций' if @routes.empty? || @stations.length <= 2
